...
public class QuickFormatter: LogFormatter {
    ...
    public init(format: QuickFormatterFormat = .All) {
        self.format = format
    }
    ...
    public func formatLog<T>(logger: Logger, level: LogLevel, @autoclosure message givenMessage: () -> T,
        filename: String?, line: Int?,  function: String?) -> String {
            var s: String;
            var dateString = RLUtilities.getLocalTimeString()
            ...
            case .DateLevelMessage:
                ...
                s = "\(dateString) \(level.label): \(message)";
            case .MessageOnly:
                s = "\(message)";
            case .NameMessage:
            ...
            case .DateMessage:
                s = "\(dateString) \(message)";
            case .All:
                s = "\(dateString) \(level.label) \(logger.name): \(message)";
            }
            return s
    }
    ...
    public func formatLog<T>(logger: Logger, level: LogLevel, @autoclosure message givenMessage: () -> T,
        filename: String?, line: Int?,  function: String?) -> String {
            ...
            for (index, part) in enumerate(format) {
                ...
                case .DATE: logMessage +=   (RLUtilities.getLocalTimeString() as String)
                case .LINE:
...
public class FileLocation: LogLocation {
    ...
    func updateFileName(){
        RLUtilities.getRLDataFormatter().dateFormat = "yyyyMMdd"
        var dateStr = RLUtilities.getRLDataFormatter().stringFromDate(NSDate())
        self.fileDateName = "rlterm3log_\(dateStr)\(filename)"
    }
    deinit {
        closeFile()
    }
    ...
    func setDirectory() {
        let temp: NSString = self.fileDateName
        if temp.rangeOfString("/").location != Foundation.NSNotFound {
            // "/" was found in the filename, so we use whatever path is already there
            if (self.fileDateName.hasPrefix("~/")) {
                self.fileDateName = self.fileDateName.stringByExpandingTildeInPath
            }
        ...
        if let dir: String = dirs as? String {
            ...
            let path = dir.stringByAppendingPathComponent(self.fileDateName);
            self.fileDateName = path;
        }
    }
    ...
    func openFile() {
        ...
        if !NSFileManager.defaultManager().fileExistsAtPath(self.fileDateName) {
            NSFileManager.defaultManager().createFileAtPath(self.fileDateName, contents: nil, attributes: nil)
        }
        fileHandle = NSFileHandle(forWritingAtPath:self.fileDateName);
    ...
    public func description() -> String {
        return "FileLocation filename=\(fileDateName)"
    }
}
...
public class Logger {
    ...
    public init(name: String,
        ...
        logLocation: LogLocation = ConsoleLocation.getInstance()) {
            ...
            var fileLocation = FileLocation.getInstance("log.txt")
            self.locations.append(fileLocation)
            RLLoggerManager.registerLogger(self);
    }
        ...
        filename: String? = __FILE__, line: Int? = __LINE__,  function: String? = __FUNCTION__) {
            if (self.enabled) && (logLevel.level >= level.level) {
                ...
                for location in locations {
                    if location is ConsoleLocation{
...
let globalSwell = RLLoggerManager();

@objc
public class RLLoggerManager {
...
let _RLLOGGER =  Logger(name:"ipos")
public func RLLOGGER()->Logger{
    return _RLLOGGER
}

let _useTestLog = true
public func RLTESTLOG(msg:NSString){
    if _useTestLog == false
    {
        return
    }
    var testMsg =  "[\(__FILE__)][\(__LINE__)][\(__FUNCTION__)]\(msg)"
    println(testMsg)
}
